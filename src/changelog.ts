...
export const CHANGELOG: Record<string, string[]> = {
  '0.20.38': [
    'Ліва акаунт-панель при наведенні розширюється до 280 px (було 200) — назви акаунтів і телефони більше не обрізаються.',
  ],
  '0.20.37': [
    'Заплановані TG-повідомлення (TG schedule) тепер видно в чаті як такі: 📅 + дата і час доставки замість «Надіслано», бабл з amber-рамкою. Більше не сортуються вгору рейлу як «новіші».',
  ],
  '0.20.36': [
    'Бізнес-rail розбито на дві підгрупи: «Viber» і «Telegram-боти». Кожна сворачивається окремо, лічильник непрочитаних показується поруч з заголовком.',
  ],
  '0.20.35': [
    'Чат бота: пересилання повідомлень в іншого пацієнта (через TG copyMessage — клієнт бачить як свіже повідомлення без явної мітки «forwarded»).',
    'Чат бота: голосові повідомлення від пацієнта автоматично транскрибуються Whisper, текст показується під bubble.',
    'Чат бота: коли пацієнт надсилає кілька фото одним пакетом (TG media-group), вони приходять зі спільним media_group_id для майбутньої grid-групування.',
    'Бекенд: пошук по історії бот-чатів (GET /api/business/search/?q=…) — фронт UI з\'явиться у наступному релізі.',
  ],
  '0.20.34': [
  ...
  '0.20.28': [
    'Meta-чати: коли Meta блокує надсилання (закінчилось 24-год вікно, немає App Review для тегу, користувач видалив акаунт, ліміт запитів, токен сторінки застарів) — у композері тепер видно зрозуміле українське пояснення з іконкою (⏰ / 🛡 / 🚫 / 👤 / ⏳ / 🔑) та підказкою «переключіться на TG/WA», а не сирий fb_error JSON.',
    'Контактна стрічка Meta-чатів вантажиться помітно швидше — backend перейшов на SQL DISTINCT ON замість пагінації по 5 000 повідомлень у Python.',
  ],
  '0.20.27': [
  ...
  '0.20.9': [
    ...
    'Instagram Direct поки заблокований Meta до App Review (3-14 днів). FB Messenger — webhook налаштований, ловить події end-to-end (бекенд готовий).',
  ],
  '0.20.8': [
    'Виправлено: лічильник прийняття ліда тепер реально паузиться поза робочими годинами клініки — рахує тільки робочі секунди, у нічний час і вихідні стоїть на місці. На картці видно «🌙 Старт о 09:00 / завтра 09:00» з конкретним часом коли клінік знов відкриється.',
    'Адмін у CC mini-app → вкладка «Робочий час» налаштовує розклад на кожен день тижня окремо + список свят. Всі лічильники одразу враховують зміни.',
  ],
  '0.20.7': [
    'На картці лід-консультації тепер показуємо «📄 Джерело» — з якої статті у мініапі пацієнт натиснув кнопку «Запис на консультацію». Адміни редагують кнопку у будь-якій статті через TipTap-редактор у CC, ставлять текст і опис джерела (наприклад «Косметологія: біоревіталізація»).',
    'Rate-limit на повторну заявку знижено з 24 годин до 1 години — пацієнт може швидше переоформити запит, якщо передумав.',
  ],
  '0.20.6': [
    'Виправлено помилку «Не вдалося прийняти» на картці лід-консультації — accept-ендпоінт падав через select_for_update без транзакції. Тепер кнопка спрацьовує миттєво.',
    'Лічильник часу на картці став контрастним: солідний кружок з білим текстом замість блідого жовтого/червоного, який зливався з фоном.',
    'Лічильник реакції тепер не нараховує час поза робочими годинами клініки (09:00–19:00 Київ) — якщо запит надійшов уночі, відлік стартує з 09:00 наступного дня. На картці видно мітку «🌙 Поза робочим часом» поки не настане ранок.',
  ],
  '0.20.5': [
    'Запит на консультацію з Telegram-мініапу пацієнта тепер у чаті — окрема картка з лічильником ⏱ часу очікування і кнопкою «ЛІД прийнято». Колір картки переходить з жовтого в червоний після 60 с, в зелений — після прийняття. Хто з операторів натиснув і скільки секунд воно лежало — все зберігається в БД (потрібно для статистики швидкості реакції).',
  ],
  '0.20.4': [
    'Виправлено відтворення аудіо Binotel-дзвінків: запис не грався через CSP-обмеження Tauri (домен cc.vidnova.app не був у `media-src`). Тепер плеєр успішно завантажує та програє запис',
  ],
  '0.20.3': [
    'Пропущені / скасовані / зайняті / без відповіді дзвінки у чаті більше не клікаються — у них немає ні запису, ні транскрипції, тому розкривати нічого. Шеврон зникає, курсор звичайний',
  ],
  '0.20.2': [
    'Fix: картки дзвінків і presence (кому видно що ти в чаті) — переплутав порядок параметрів у authFetch(url, token), через що токен не надсилався у заголовку і бекенд відповідав 401/403. Тепер транскрипція відкривається і presence heartbeat працює',
  ],
  '0.20.1': [
    'Картки дзвінків у чатах тепер розкриваються: клік на картку — плавно відкривається панель з транскрипцією (оператор — синім, клієнт — смарагдовим) і красивим плеєром з play/pause, прогрес-баром і таймером',
    'Запис не запускається автоматично — тільки по кнопці «Прослухати». Якщо дзвінок ще не оброблений системою — показується пояснення «транскрипція недоступна» + плеєр (можна просто послухати запис)',
    'Транскрипція кешується в пам\'яті — повторне відкриття тієї самої картки миттєве, без запиту на сервер',
  ],
  '0.20.0': [
    'Хто дивиться / пише в чаті (presence): над полем вводу зʼявляється тонка плашка «Настя відкрила чат» або «Настя набирає…» коли інший оператор з вашого акаунта вже працює з цим пацієнтом. Формат списку під час мікса: «Настя (набирає), Олена»',
    'У списку контактів — маленька пульсуюча точка біля аватарки контакта, коли з ним уже хтось з ваших колег працює. Синя = дивиться, зелена = набирає. Hover показує імена',
    'Ніяких блокувань немає: видно хто що робить, але ви завжди можете написати. Сервер тримає presence 30с з TTL — якщо оператор закрив ноут без виходу, його автоматично приберуть зі списку',
  ],
  '0.19.7': [
    'Налаштування → Сповіщення: іконки Telegram та WhatsApp акаунтів тепер у рідних бренд-кольорах (блакитний TG, зелений WA), як і Gmail / Viber / TG-bot / Facebook / Instagram. Раніше вони були сірими з opacity 50%',
  ],
  '0.19.6': [
    'Налаштування → Сповіщення: бізнес-чати та соцмережі (Viber, Telegram-bot, Facebook, Instagram) тепер мають окреме керування сповіщеннями — свій дзвіночок (тост у куті екрана), динамік (звук) та вибір мелодії, як у звичайних TG/WA акаунтів',
    'Дружелюбні підказки при наведенні на дзвіночок/динамік: тепер видно, що саме вмикає кожен перемикач (тост у куті екрана або аудіо-сигнал). Підказки у стилі frosted-glass застосунку',
  ],
  '0.19.5': [
  ...
  '0.19.3': [
    'Режим "Усі месенджери": клік на контакт з листуваннями в кількох каналах (наприклад, TG + WhatsApp + Viber) тепер відкриває модалку «Оберіть акаунт» з датою останнього повідомлення у кожному. Якщо канал один — чат відкривається одразу. Якщо акаунт уже обраний у сайдбарі — відкриваємо в ньому без питань',
  ],
  '0.19.2': [
    'Режим "Всі месенджери": список контактів тепер завантажує всі записи (infinite scroll), а не лише перші 50. Причина обмеження — backend повідомляв розмір уже після дедуплікації TG+WA на сторінці, тож фронт думав що більше нічого немає. Тепер віддає реальний total',
  ],
  '0.19.1': [
    'Режим "Всі акаунти" + глобальний пошук по повідомленнях: клік на знайдений результат тепер надійно перемикає на потрібний акаунт і відкриває діалог, навіть якщо backend з якоїсь причини не повернув account_id (fallback — перший TG або WA акаунт залежно від source)',
  ],
  '0.19.0': [
    'Плутанина імен виправлена: у всіх чатах (TG/WA/Viber/FB/IG) відображається full_name з CRM (Binotel/KeyCRM), а tg_name використовується лише як fallback коли ПІБ з CRM порожнє. Приклад: 0683868351 — CRM пише "Анастасія", tg_name застарілий "Євгенія Руденко" з чужого TG-імпорту — тепер скрізь "Анастасія"',
  ],
  '0.18.9': [
    'Viber/бізнес-чати: після відправки свого повідомлення чат автоматично прокручується вниз — нове повідомлення більше не ховається за вікном вводу',
    'Бренд-кольори бульб: Telegram-bot тепер блакитний (як звичайний TG), Facebook Messenger — синій, Instagram — рожево-оранжевий градієнт. Раніше всі бізнес-повідомлення були фіолетовими (Viber), тепер кожен месенджер має свій колір',
  ],
  '0.18.8': [
    'Viber/бізнес-чати: при відкритті чату скрол автоматично йде до останнього (найновішого) повідомлення — як у TG/WA. Раніше треба було скролити вниз вручну. Також робиться при появі нових повідомлень через polling',
  ],
  '0.18.7': [
    'Під кожним повідомленням у чаті тепер компактний бейдж «звідки/хто». Для вхідних — іконка месенджера + назва акаунту (тільки коли це НЕ поточний активний акаунт). Для вихідних — ім\'я оператора, і додатково акаунт якщо повідомлення з іншого каналу. Бот — «🤖 Bot». Старі повідомлення без автора відображаються без бейджа оператора',
  ],
  '0.18.6': [
    'Співробітники клініки у бізнес-чатах (Viber / Telegram-bot / Meta) тепер підсвічуються фіолетовим, як у Telegram/WhatsApp — однакова візуальна ознака співробітника у всіх каналах',
    'Склейка історії у чатах — лише для пацієнтів. У чатах зі співробітниками показується лише історія того каналу, у якому відкрито чат (не мішаємо внутрішнє спілкування співробітників між різними каналами). Для пацієнтів — повний мерж як був',
  ],
  '0.18.5': [
    'Viber-повідомлення у чаті тепер у фіолетовій підкладці (бренд Viber), WhatsApp — зелена, Telegram — стандартна. Видно з першого погляду, з якого каналу повідомлення',
    'Кроссмесенджерна історія тепер двостороння: Viber-переписка з клієнтом показується і в його TG/WA-чатах, і навпаки (мерж за номером + linked phones). Повний таймлайн контакту в будь-якому чаті',
  ],
  '0.18.4': [
    'Фікс: клік на toast Viber/бізнес-акаунта тепер надійно переходить у потрібний чат навіть при race — якщо toast прилетів раніше ніж завантажився список businessAccounts, VG вважає невідомий accountId бізнесом (раніше падав у TG-шлях і «губив» toast)',
  ],
  '0.18.3': [
    'Авторизація триває один календарний день: увійшов уранці — до 00:00 VG більше не питає логін; після півночі — потрібен новий вхід. Якщо додаток був відкритий через північ, VG автоматично виходить. Працює в локальному часовому поясі (00:00–24:00)',
  ],
  '0.18.2': [
    'КРИТИЧНИЙ фікс link-file flow: Tauri v2 блокує нативний `window.confirm()` через ACL, тому діалог «Надіслати як посилання?» не показувався і upload падав з 500 на бекенді (`UnboundLocalError: mime`). Замінив на власну модалку ConfirmDialog + виправив регресію з змінною `mime` у upload-media',
    'Viber: повідомлення з кнопкою тепер видно в чаті VG — раніше backend повертав `reply_markup: null` (фікс попереднього крашу), VG не мав звідки взяти caption+action. Додано `button_text`/`button_url` у серіалізатор, кнопка рендериться під текстом фіолетовою плашкою (клік — відкриває URL через shell)',
    'Viber: час вхідних повідомлень коректний — TurboSMS надсилав `received_date` у київському часі без TZ, ми парсили як UTC і повідомлення стрибали на +2-3 години вперед. Тепер використовуємо реальний UTC з моменту отримання webhook',
  ],
  '0.18.1': [
    'Viber: кнопка-посилання тепер у модальному вікні. Біля композера з\'явилась іконка 🔗 (тултіп «Надіслати повідомлення з кнопкою»). Клік → модалка з трьома полями: текст, назва кнопки, URL → Надіслати. Більше не займає місце у чаті',
    'Toast-сповіщення Viber: клік на toast тепер переходить у бізнес-акаунт і відкриває саме той Viber-чат (раніше toast не знав про бізнес-акаунти)',
    'Toast-сповіщення Viber: колір фіолетовий (як в Viber) замість загального синього — бекенд шле business_account_id замість account_id, тепер VG читає обидва',
  ],
  '0.18.0': [
    'Viber: можна додати до повідомлення кнопку-посилання. У чаті під композером «🔗 Додати кнопку-посилання» → текст кнопки (до 30 символів) + URL. Клієнт побачить під повідомленням клікабельну кнопку (TurboSMS action + caption)',
    'Viber: можна надсилати PDF/відео/voice — якщо файл не JPEG/PNG або більше 1 МБ, VG пропонує «Надіслати як посилання». Файл завантажується на наш сервер, клієнту у Viber приходить пояснення + кнопка «Завантажити» з захищеним підписаним URL. У чаті VG повідомлення відображається як надісланий документ',
    'Viber: жорстка валідація перед upload — замість тихого 415 від сервера показується чіткий діалог з поясненням обмежень Viber Business',
  ],
  '0.17.99': [
  ...
  '0.17.98': [
    ...
    'WhatsApp: ✓✓ (прочитано) працює реально — відображається після того як співрозмовник відкрив чат (astra polling ack≥3 → webhook /bridge/read/ → WS read_outbox)',
  ],
  '0.17.97': [
    'Viber "Новий чат": список клієнтів у стилі додатка (аватар + ПІБ + телефон, hover). Контакти, з якими вже були Viber-повідомлення, позначені фіолетовим бейджем Viber — їхній номер точно доступний у Viber',
    'Viber "Новий чат": після вибору клієнта модалка закривається і відкривається звичайний чат — писати можна прямо з композера внизу (фото, текст — як у будь-якому чаті)',
  ],
  '0.17.96': [
  ...
  '0.17.95': [
    ...
    'Фікс: коли обрано бізнес-акаунт, кнопка "Новий чат" більше не просить обрати акаунт — в бізнес-месенджерах клієнт має написати першим, тож кнопка прихована з підказкою',
  ],
  '0.17.94': [
  ...
  '0.17.92': [
    'Viber-чат: нові повідомлення тепер знизу (як у Telegram/WhatsApp). Раніше порядок був зворотний — нові зверху',
    'Кнопка "Новий чат": якщо обрано конкретний акаунт у сайдбарі — одразу створює чат на ньому без додаткового вибору. Селектор акаунту показується тільки в режимі "Всі акаунти"',
    'Toast-сповіщення тепер у бренд-кольорах месенджера: Viber — фіолетовий, Telegram-bot — блакитний, Facebook Messenger — синій, Instagram — градієнт. Gmail, TG, WA — без змін',
  ],
  '0.17.91': [
